package com.shevelev.wizard_camera.core.camera_gl.impl.shared.factory
...
import com.shevelev.wizard_camera.core.camera_gl.impl.bitmap.filters.GlSurfaceShaderFilterImpl
import com.shevelev.wizard_camera.core.common_entities.enums.GlFilterCode
import com.shevelev.wizard_camera.core.common_entities.filter_settings.gl.*
import com.shevelev.wizard_camera.core.camera_gl.impl.shared.*
import com.shevelev.wizard_camera.core.utils.resources.getScreenSize

/**
 * Creates filters
 */
internal class GlShaderFiltersFactoryImpl(
    private val context: Context
) : GlShaderFiltersFactory {
    /**
     * Create an OGL wrapper around a filter based on its settings
     */
    override fun createGLFilterSettings(settings: GlFilterSettings, context: Context) : GLFilterSettings =
        when(settings.code) {
    ...
    override fun createFilter(bitmap: Bitmap, settings: GlFilterSettings): GlSurfaceShaderFilter {
        return GlSurfaceShaderFilterImpl(
            context,
            bitmap,
            getFilterRes(settings.code),
            context.getScreenSize(),
            createGLFilterSettings(settings, context)
        )
    }

    @RawRes
    override fun getFilterRes(code: GlFilterCode): Int =
        when(code) {
